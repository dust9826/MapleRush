...
## Acceptance criteria
- [x] 자쿰(BossKind="zakum")이 플레이어를 추격해 좌우로 이동하지 않는다.
- [x] 자쿰의 3패턴(P-Z1 근접/P-Z2 수평탄/P-Z3 다지점)은 정상 발동한다.
- [x] 엘리쟈/스노우맨 등 다른 보스의 추격 동작은 그대로 유지된다(기본값 false).

## Subtasks
- [x] BossController에 `Stationary` 불리언 프로퍼티 추가 + 추격 블록 게이팅
- [x] BossZakum 모델에 `Stationary=true` 주입(BossKind와 동일한 Value 방식)
- [x] refresh → build 로그 확인(에러 0)
- [x] play로 자쿰 이동 X 검증(사용자 확인)
- [x] PR #48 생성(master 기준)

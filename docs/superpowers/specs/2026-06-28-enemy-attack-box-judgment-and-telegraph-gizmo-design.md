...
적 공격의 명중 판정을 **자기 중심 원**에서 **사각형(box)**으로 바꾼다 — 근접(평타·보스 slam)은 적이 바라보는 방향 앞으로 뻗는 **전방 박스**, 보스 광역(redSmash)은 가로로 넓고 세로로 낮은 **중앙 박스**(점프/위-대시로만 회피). 더불어 모든 적 공격의 **예상 공격 위치를 디버그 기즈모로 정확히** 표시한다 — 근접/광역=박스, 원거리=회전 직사각형 레인(멀티샷이면 여러 갈래).
...
> 적 공격 레코드 마커는 **전부 박스**(노랑 전방 / 빨강 중앙)로 통일 — 적공격 원 홀더 불필요. 원 마커는 투사체(주황)·플레이어공격플래시(하늘)·대시(사거리/마커)만 사용.
...
- **적 공격 마커 풀 재구성**: 현 atkY/atkR(단위원, 인덱스 15~26)을 **박스 홀더**로 전환. 적 공격 박스(노랑 N + 빨강 M) + **레인용 노랑 박스 홀더 K** 추가. 박스/레인 모두 단위정사각 홀더(rot+scale)라 동형. 새 인덱스 레이아웃·풀 크기는 구현 시 확정, `GizmoLinePoolSize` 갱신.
- 신규 메서드:
...
- 적공격 원 홀더(atkY/atkR 원)는 제거 — 모든 적 공격 레코드가 박스가 되므로 불필요.
